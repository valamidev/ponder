...
// dir.
export const addStackTrace = (error, options) => {
    if (!error.stack)
        return;
    ...
    const firstUserFrameIndex = stackTrace.findIndex((frame) => frame.file?.includes(options.indexingDir));
    if (firstUserFrameIndex >= 0) {
        userStackTrace = stackTrace.filter((frame) => frame.file?.includes(options.indexingDir));
        ...
        if (firstUserFrame?.file && firstUserFrame?.lineNumber) {
            try {
                ...
                codeFrame = codeFrameColumns(sourceContent, {
                    start: {
    ...
    const formattedStackTrace = [
        ...
        ...userStackTrace.map(({ file, lineNumber, column, methodName }) => {
            const prefix = "    at";
            const path = `${file}${lineNumber !== null ? `:${lineNumber}` : ""}${column !== null ? `:${column}` : ""}`;
...
//# sourceMappingURL=addStackTrace.js.map
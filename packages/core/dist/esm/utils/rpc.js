...
import { BlockNotFoundError, TransactionReceiptNotFoundError, numberToHex, zeroAddress, zeroHash, } from "viem";
/**
...
export const _eth_getTransactionReceipt = (rpc, { hash }) => rpc
    ...
    return standardizeTransactionReceipt(receipt);
});
/**
 ...
 */
export const validateTransactionsAndBlock = (block) => {
    const transactionIds = new Set();
    for (const transaction of block.transactions) {
        if (block.hash !== transaction.blockHash) {
            throw new Error(`Detected inconsistent RPC responses. 'transaction.blockHash' ${transaction.blockHash} does not match 'block.hash' ${block.hash}`);
        }
        if (block.number !== transaction.blockNumber) {
            throw new Error(`Detected inconsistent RPC responses. 'transaction.blockNumber' ${transaction.blockNumber} does not match 'block.number' ${block.number}`);
        }
        if (transactionIds.has(transaction.transactionIndex)) {
            throw new Error(`Detected invalid eth_getBlock response. Duplicate transaction index ${transaction.transactionIndex} for block ${block.hash}.`);
        }
        else {
 ...
 */
export const validateLogsAndBlock = (logs, block) => {
    ...
    for (const log of logs) {
        ...
        if (block.number !== log.blockNumber) {
            throw new Error(`Detected inconsistent RPC responses. 'log.blockNumber' ${log.blockNumber} does not match 'block.number' ${block.number}`);
        }
        if (log.transactionHash !== zeroHash) {
            const transaction = transactionByIndex.get(log.transactionIndex);
            if (transaction === undefined) {
                throw new Error(`Detected inconsistent RPC responses. 'log.transactionIndex' ${log.transactionIndex} not found in 'block.transactions' for block ${block.hash}`);
            }
            else if (transaction.hash !== log.transactionHash) {
                throw new Error(`Detected inconsistent RPC responses. 'log.transactionHash' ${log.transactionHash} does not match 'transaction.hash' ${transaction.hash} for block ${block.hash}`);
            }
        }
        if (logIndexes.has(log.logIndex)) {
            throw new Error(`Detected invalid eth_getLogs response. Duplicate log index ${log.logIndex} for block ${log.blockHash}.`);
        }
        else {
 ...
 */
export const validateTracesAndBlock = (traces, block) => {
    ...
    for (const trace of traces) {
        ...
        if (traceIds.has(id)) {
            throw new Error(`Detected invalid debug_traceBlockByHash response. Duplicate trace index ${trace.trace.index} for transaction ${trace.transactionHash}.`);
        }
        else {
            traceIds.add(id);
        }
 ...
 */
export const validateReceiptsAndBlock = (receipts, block, method) => {
    const receiptIds = new Set();
    for (const receipt of receipts) {
        const id = receipt.transactionHash;
        if (receiptIds.has(id)) {
            throw new Error(`Detected invalid eth_getBlockReceipts response. Duplicate receipt for transaction ${receipt.transactionHash}.`);
        }
        else {
            receiptIds.add(id);
        }
    ...
    for (const receipt of receipts) {
        if (block.hash !== receipt.blockHash) {
            throw new Error(`Detected inconsistent RPC responses. 'receipt.blockHash' ${receipt.blockHash} does not match 'block.hash' ${block.hash}`);
        }
        if (block.number !== receipt.blockNumber) {
            throw new Error(`Detected inconsistent RPC responses. 'receipt.blockNumber' ${receipt.blockNumber} does not match 'block.number' ${block.number}`);
        }
        const transaction = transactionByIndex.get(receipt.transactionIndex);
        if (transaction === undefined) {
            throw new Error(`Detected inconsistent RPC responses. 'receipt.transactionIndex' ${receipt.transactionIndex} not found in 'block.transactions' for block ${block.hash}`);
        }
        else if (transaction.hash !== receipt.transactionHash) {
            throw new Error(`Detected inconsistent RPC responses. 'receipt.transactionHash' ${receipt.transactionHash} does not match 'transaction.hash' ${transaction.hash} for block ${block.hash}`);
        }
    }
    if (method === "eth_getBlockReceipts" &&
        block.transactions.length !== receipts.length) {
        throw new Error(`Detected inconsistent RPC responses. 'block.transactions' length ${block.transactions.length} does not match 'receipts' length ${receipts.length} for block ${block.hash}`);
    }
};
/**
 ...
 * - extraData
 */
export const standardizeBlock = (block, isBlockHeader = false) => {
    // required properties
 ...
 * - gas
 */
export const standardizeTransaction = (transaction) => {
    // required properties
 ...
 * - removed
 */
export const standardizeLog = (log) => {
    // required properties
 ...
 * - gasUsed
 */
export const standardizeTrace = (trace) => {
    // required properties
 ...
 * - type
 */
export const standardizeTransactionReceipt = (receipt) => {
    // required properties

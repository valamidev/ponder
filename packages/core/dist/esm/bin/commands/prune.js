...
export async function prune({ cliOptions }) {
    ...
    const ponderSchemas = await database
        ...
        .from(TABLES)
        .where(inArray(TABLES.table_schema, database
        .adminQB()
    ...
    const queries = ponderSchemas.map((row) => database
        .adminQB()
        .select({
    ...
    if (tablesToDrop.length > 0) {
        await database
            .adminQB()
            .execute(sql.raw(`DROP TABLE IF EXISTS ${tablesToDrop.join(", ")} CASCADE`));
        logger.warn({
            service: "prune",
    ...
    if (viewsToDrop.length > 0) {
        await database
            .adminQB()
            .execute(sql.raw(`DROP VIEW IF EXISTS ${viewsToDrop.join(", ")} CASCADE`));
        logger.warn({
            service: "prune",
    ...
    if (functionsToDrop.length > 0) {
        await database
            .adminQB()
            .execute(sql.raw(`DROP FUNCTION IF EXISTS ${functionsToDrop.join(", ")} CASCADE`));
        logger.warn({
            service: "prune",
    ...
    if (schemasToDrop.length > 0) {
        await database
            .adminQB()
            .execute(sql.raw(`DROP SCHEMA IF EXISTS ${schemasToDrop.join(", ")} CASCADE`));
        logger.warn({
            service: "prune",

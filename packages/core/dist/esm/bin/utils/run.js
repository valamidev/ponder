...
import { commitBlock, createIndexes, createTrigger, dropTrigger, finalize, revert, } from '../../database/utils.js';
...
export async function run({ common, preBuild, namespaceBuild, schemaBuild, indexingBuild, crashRecoveryCheckpoint, database, onFatalError, onReloadableError, }) {
            ...
            await database
                ...
                for (const eventChunk of eventChunks) {
                    ...
                    if (preBuild.ordering === "multichain") {
                        ...
                        common.metrics.ponder_historical_completed_indexing_seconds.set({ chain: chain.name }, Math.max(Number(checkpoint.blockTimestamp) -
                            Math.max(sync.seconds[chain.name].cached, sync.seconds[chain.name].start), 0));
                        ...
                        for (const chain of indexingBuild.chains) {
                            common.metrics.ponder_historical_completed_indexing_seconds.set({ chain: chain.name }, Math.min(Math.max(Number(checkpoint.blockTimestamp) -
                                Math.max(sync.seconds[chain.name].cached, sync.seconds[chain.name].start), 0), Math.max(sync.seconds[chain.name].end -
                ...
                if (events.checkpoints.length > 0) {
                    await database
                        .adminQB("update_checkpoints")
                        .insert(PONDER_CHECKPOINT)
                        .values(events.checkpoints.map(({ chainId, checkpoint }) => ({
    ...
    await Promise.all(tables.map((table) => createTrigger(database.adminQB, { table })));
    if (namespaceBuild.viewsSchema) {
        await database.adminQB("create_views").transaction(async (tx) => {
            await tx().execute(sql.raw(`CREATE SCHEMA IF NOT EXISTS "${namespaceBuild.viewsSchema}"`));
            for (const table of tables) {
                // Note: drop views before creating new ones to avoid enum errors.
                await tx().execute(sql.raw(`DROP VIEW IF EXISTS "${namespaceBuild.viewsSchema}"."${getTableName(table)}"`));
                await tx().execute(sql.raw(`CREATE VIEW "${namespaceBuild.viewsSchema}"."${getTableName(table)}" AS SELECT * FROM "${namespaceBuild.schema}"."${getTableName(table)}"`));
            }
            common.logger.info({
                service: "app",
                msg: `Created ${tables.length} views in schema "${namespaceBuild.viewsSchema}"`,
            });
            await tx().execute(sql.raw(`CREATE OR REPLACE VIEW "${namespaceBuild.viewsSchema}"."_ponder_meta" AS SELECT * FROM "${namespaceBuild.schema}"."_ponder_meta"`));
            await tx().execute(sql.raw(`CREATE OR REPLACE VIEW "${namespaceBuild.viewsSchema}"."_ponder_checkpoint" AS SELECT * FROM "${namespaceBuild.schema}"."_ponder_checkpoint"`));
            ...
            case "block": {
                if (event.events.length > 0) {
                    ...
                    for (const { checkpoint, events } of perBlockEvents) {
                        ...
                        if (result.status === "error")
                            onReloadableError(result.error);
                        await Promise.all(tables.map((table) => commitBlock(database.userQB, { table, checkpoint })));
                        if (preBuild.ordering === "multichain") {

...
import { _eth_getBlockByHash, _eth_getBlockByNumber, standardizeBlock, } from '../utils/rpc.js';
import { startClock } from '../utils/timer.js';
import { wait } from '../utils/wait.js';
import { getLogsRetryHelper, } from "@ponder/utils";
import { http, HttpRequestError, JsonRpcVersionUnsupportedError, MethodNotFoundRpcError, MethodNotSupportedRpcError, ParseRpcError, TimeoutError, isHex, webSocket, } from "viem";
const RETRY_COUNT = 9;
...
const addRequestTimestamp = (bucket) => {
...
const getRPS = (bucket) => {
    ...
    if (bucket.requestTimestamps.length === 0)
        return 0;
    const t = bucket.requestTimestamps[bucket.requestTimestamps.length - 1] -
...
const isAvailable = (bucket) => {
    if (bucket.isActive && getRPS(bucket) < bucket.rpsLimit)
        return true;
...
const increaseMaxRPS = (bucket) => {
    if (bucket.consecutiveSuccessfulRequests >= SUCCESS_WINDOW_SIZE &&
        getRPS(bucket) > bucket.rpsLimit * RPS_INCREASE_QUALIFIER) {
        const newRPSLimit = Math.min(bucket.rpsLimit * RPS_INCREASE_FACTOR, MAX_RPS);
        bucket.rpsLimit = newRPSLimit;
        bucket.consecutiveSuccessfulRequests = 0;
    }
};
const decreaseMaxRPS = (bucket) => {
...
export const createRpc = ({ common, chain, concurrency = 25, }) => {
    ...
    const buckets = request.map((request, index) => ({
        index,
